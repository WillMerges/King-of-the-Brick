...
bool isIndexAttacked(BoardInfo* b, U8 i, bool whiteToMove) {
	...
	if ( ( (whiteToMove ? whitePawn[i] : blackPawn[i])
			& (b->whitePieces[PAWN] | b->blackPieces[PAWN]) & others) != 0){
		return true;
	}
	if ( (king[i] & (b->whitePieces[KING] | b->blackPieces[KING]) & others) != 0){
		return true;
	}
	if ( (knight[i] & (b->whitePieces[KNIGHT] | b->blackPieces[KNIGHT]) & others) != 0){
		return true;
	}
	if ( (getRookAttacks(i, all)
			& ( (b->whitePieces[ROOK] | b->blackPieces[ROOK]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN])) & others) != 0){
		return true;
	}
	if ( (getBishopAttacks(i, all)
			& ( (b->whitePieces[BISHOP] | b->blackPieces[BISHOP]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN])) & others) != 0){
		return true;
	}
	return false;
}
...
U64 getIndexAttacks(BoardInfo* b, int i) {
	...
	return  ((((b->BlackPiecesBB & whitePawn[i])
			| (b->WhitePiecesBB& blackPawn[i])) & (b->whitePieces[PAWN] | b->blackPieces[PAWN]))
			| (king[i] & (b->whitePieces[KING] | b->blackPieces[KING]))
			| (knight[i] & (b->whitePieces[KNIGHT] | b->blackPieces[KNIGHT]))
			| (getRookAttacks(i, all) & ( (b->whitePieces[ROOK] | b->blackPieces[ROOK]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN])))
			| (getBishopAttacks(i, all) & ( (b->whitePieces[BISHOP] | b->blackPieces[BISHOP]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN]))));
}
/*
...
U64 getXrayAttacks(BoardInfo* b, int i, U64 all) {
	...
	return ( (getRookAttacks(i, all) & ( (b->whitePieces[ROOK] | b->blackPieces[ROOK]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN]))) | (getBishopAttacks(
			i, all) & ( (b->whitePieces[BISHOP] | b->blackPieces[BISHOP]) | (b->whitePieces[QUEEN] | b->blackPieces[QUEEN]))))
			& all;
}
